def AutoPartes(ventas: list):
    ...
    for idProducto, dProducto, pnProducto, cvProducto, sProducto, \
        ...
        # Forzar la entrada
        if dict_Ventas.get(idProducto) == None:
            # Creamos item de lista
            dict_Ventas[idProducto] = []
        
        # Registar una nueva lista
        dict_Ventas[idProducto].append((dProducto, pnProducto, cvProducto, sProducto, \
            nComprador, cComprador, fVenta))
    
    return dict_Ventas

def consultaRegistro(ventas, idProducto):
    # Ubicar una idProducto
    if idProducto in ventas:
        for dProducto, pnProducto, cvProducto, sProducto, nComprador, \
            cComprador, fVenta in ventas[idProducto]:

            # Mostrar todos los registros encontrador
